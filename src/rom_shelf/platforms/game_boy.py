...
class GameBoyPlatform(BasePlatform):
    ...
    def get_platform_settings(self) -> list[PlatformSetting]:
        ...
        return [
            PlatformUtils.create_rom_directories_setting("Game Boy"),
            PlatformUtils.create_scan_subdirectories_setting(),
            PlatformUtils.create_supported_formats_setting("Game Boy", [".gb"]),
    ...
    def validate_rom(self, file_path: Path) -> bool:
        ...
        if not PlatformUtils.validate_file_exists_and_extension(file_path, [".gb"]):
            return False

        # Basic size check - Game Boy ROMs are typically 32KB to 8MB
