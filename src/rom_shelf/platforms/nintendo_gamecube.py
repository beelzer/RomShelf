...
class NintendoGameCubePlatform(BasePlatform):
    ...
    def get_platform_settings(self) -> list[PlatformSetting]:
        ...
        return [
            ...
            PlatformUtils.create_supported_formats_setting(
                "Nintendo GameCube", [".iso", ".gcm", ".rvz", ".wbfs", ".ciso"]
            ),
    ...
    def validate_rom(self, file_path: Path) -> bool:
        ...
        if not exists_and_ext:
            print(
                f"DEBUG: Failed exists/extension check - exists: {file_path.exists()}, extension: {file_path.suffix}"
            )
            return False
        ...
        print(
            f"DEBUG: Size validation: {size_valid} (size: {file_path.stat().st_size / 1024 / 1024:.1f} MB)"
        )
